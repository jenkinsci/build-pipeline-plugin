...
public final class QueueUtil {
    ...
    public static final HashMap<String, QueueEntry> QUEUE_ENTRIES = new HashMap<String, QueueEntry>();

    /**
     * Adding an entry into the list of queued items.
     * @param queueEntry - entry to be added
     */
     ...
     * 
     * @param project - the project to search queued items for
     ...
     * 
     * @param project - the project to search queued items for
     * @param oldItems - list of items to check the new list against
    ...
    public static int getNewQueuedItemId(AbstractProject<?, ?> project, List<Queue.Item> oldItems) {
        List<Queue.Item> newItems = Jenkins.getInstance().getQueue().getItems(project);
       
        // Now search the new list for items that are not in the old list, if one does exist, return it's id
     ...
     * 
     * @param project - the project to search queued items for
     * @param upstreamBuild the upstream build we derive the queue item from
     * @return WaitingItem - the waiting item from the queue for the given upstreamBuild
     */
     ...
     * 
     * @param project - the project to search queued items for
     * @param queueId - the id of the queued item
     * @return Item - the item from the queue for the given name
     */
